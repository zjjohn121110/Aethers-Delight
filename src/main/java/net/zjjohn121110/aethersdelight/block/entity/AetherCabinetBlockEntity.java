...
public class AetherCabinetBlockEntity extends RandomizableContainerBlockEntity {
    ...
    private ContainerOpenersCounter openersCounter = new ContainerOpenersCounter()
    {
        protected void onOpen(Level level, BlockPos pos, BlockState state) {
            AetherCabinetBlockEntity.this.playSound(state, ModSounds.BLOCK_CABINET_OPEN.get());
            AetherCabinetBlockEntity.this.updateBlockState(state, true);
        }

        protected void onClose(Level level, BlockPos pos, BlockState state) {
            AetherCabinetBlockEntity.this.playSound(state, ModSounds.BLOCK_CABINET_CLOSE.get());
    ...
    protected Component getDefaultName() {
        return Component.translatable("container." + AethersDelight.MODID + ".drawer");
    }
    ...
    void updateBlockState(BlockState state, boolean open) {
        if (level != null) {
            this.level.setBlock(this.getBlockPos(), state.setValue(AetherCabinetBlock.OPEN, open), 3);
        }
    }
    ...
    private void playSound(BlockState state, SoundEvent sound) {
        ...
        Vec3i drawerFacingVector = state.getValue(AetherCabinetBlock.FACING).getNormal();
